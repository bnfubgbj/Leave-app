...
if menu == "📝 ยื่นคำขอลา":
    ...
    if emp:
        ...
        if leave_type == "ลาพักร้อน":
            max_days = left_annual
        elif leave_type == "ลากิจ":
            max_days = left_personal
        else:
            max_days = left_sick

        st.markdown("**เลือกวันที่ต้องการลา** (คลิกที่วันในปฏิทิน)")
        st.caption(f"สิทธิ์คงเหลือ: {max_days} วัน")

        # โหลดวันที่ลาที่มีอยู่แล้ว เพื่อแสดงในปฏิทิน
        existing_leaves = load_leaves()
        my_leaves = [l for l in existing_leaves
        ...
        for l in my_leaves:
            existing_events.append({
                "title": f"{'⏳' if l.get('สถานะ','') == 'รออนุมัติ' else '✅'} {l.get('ประเภท','')}",
                "start": l.get("วันเริ่ม",""),
                "end": l.get("วันสิ้นสุด",""),
                "color": "#FFA500" if l.get("สถานะ","") == "รออนุมัติ" else "#4CAF50",
            })
        ...
        # แสดงปฏิทิน
        selected_events = [{"title": "🔴 เลือกลา", "start": d, "end": d, "color": "#E53935"}
        ...
        cal_result = st_calendar(
            events=existing_events + selected_events,
        ...
        # แสดงวันที่เลือก
        selected_dates = sorted(st.session_state.selected_dates)
        total_days = len(selected_dates)
        has_overlap = False

        if selected_dates:
            st.markdown(f"**วันที่เลือก:** {', '.join(selected_dates)}")
            if total_days > max_days:
                st.error(f"❌ คุณลาเกินสิทธิ์! เลือก {total_days} วัน แต่มีสิทธิ์แค่ {max_days} วัน")
            else:
                st.success(f"✅ เลือก **{total_days} วัน** (เหลือสิทธิ์ {max_days - total_days} วัน)")

            # เช็ควันซ้ำ
            for d in selected_dates:
                from datetime import datetime as dt
                d_date = dt.strptime(d, "%Y-%m-%d").date()
                overlap = check_overlap(emp["รหัส"], d_date, d_date)
                if overlap:
                    has_overlap = True
                    st.warning(f"⚠️ วัน {d} ซ้ำกับวันลาที่มีอยู่แล้ว!")
        else:
            st.info("คลิกวันในปฏิทินเพื่อเลือกวันลา")

        if st.button("🗑 ล้างวันที่เลือกทั้งหมด"):
            st.session_state.selected_dates = []
            st.rerun()
        ...
        days = total_days
        date_ranges = [(date.fromisoformat(d), date.fromisoformat(d), 1) for d in selected_dates]

        with st.form("leave_form"):
            st.markdown("**ยืนยันการลา**")
            if selected_dates:
                st.info(f"ประเภท: {leave_type} | จำนวน: {total_days} วัน")
